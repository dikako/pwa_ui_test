...
public class Test_Homepage extends Setup {
	...
	@DataProvider
	public String[][] homepage_search_list_tab() throws InvalidFormatException, IOException {
		ReadExcel read = new ReadExcel();
		return read.getCellData(path, "Tab Search");
	}
...
//		Thread.sleep(5000);
//		
//		alert.urlValidate("explores/search");
//		Thread.sleep(5000);
//	}
//
//	@Severity(SeverityLevel.CRITICAL)
...
//	}
	...
	@Severity(SeverityLevel.CRITICAL)
	@Description("Homepage Test - validate Search List Tab")
	@Test(priority = 0, testName = "Hompage Test - Search Direct", dataProvider = "homepage_search_list_tab")
	public void homepage_tab_on_search_page(String index, String tabsName) throws InterruptedException {
		Url url = new Url(driver);
		Button button = PageFactory.initElements(driver, Button.class);
		Alert alert = PageFactory.initElements(driver, Alert.class);

		System.out.println("Hompage Test - Search List Tab");
		...
		Integer indexToInt = Integer.valueOf(index);
		alert.byTagGetTextIndex("li", indexToInt, tabsName);
	}
}
